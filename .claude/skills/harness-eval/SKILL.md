---
name: harness-eval
description: This skill should be used when the user asks to "test the harness", "run integration tests", "validate features with real API", "test with real model calls", "run agent loop tests", "verify end-to-end", or needs to verify Codeless features on a real codebase with actual LLM calls.
version: 0.2.0
---

# Harness Eval — End-to-End Feature Validation

Validate Codeless features by running real agent loops against an unfamiliar codebase with actual LLM API calls. Every test exercises the full stack: API client → model → tool calls → execution → result.

## Core Principles

1. **Test on an unfamiliar project** — never test on Codeless itself (the agent modifies its own code). Clone a real project as the workspace.
...
Clone an unfamiliar project (do not use Codeless):
...
Then run a minimal smoke check through Codeless, not just raw `srt`, so you verify the real adapter path:

```python
from pathlib import Path
from codeless.config.settings import Settings, SandboxSettings, save_settings
from codeless.tools.bash_tool import BashTool

cfg = Path("/tmp/codeless-sandbox-settings.json")
...
- uses real Codeless engine/tool objects
...
- Distinguish environment failures from product failures
  - Example: missing dependency in the unfamiliar target repo is not automatically an Codeless regression
...
- Testing on Codeless itself — agent modifies its own running code
...
- Declaring sandbox “tested” after only checking raw `srt` — verify the Codeless adapter path too
...
- **`references/feature-matrix.md`** — Detailed test cases for every Codeless module
