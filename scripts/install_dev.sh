...
for arg in "$@"; do
    case "$arg" in
        ...
        --help|-h)
            ...
            echo "registers codeless/clh in ~/.local/bin."
            echo ""
            echo "  default         use ./ .codeless-venv inside the current repo"
            echo "  --global-venv   use ~/.codeless-venv but still install the current repo"
...
if [ "$GLOBAL_VENV" = true ]; then
    ...
    VENV_DIR="$REPO_ROOT/.codeless-venv"
fi
...
success "Installed Codeless from ${REPO_ROOT}"
...
echo "  Command links:       $BIN_DIR/codeless , $BIN_DIR/clh"
echo ""
