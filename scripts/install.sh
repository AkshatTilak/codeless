#!/usr/bin/env bash
# Codeless one-click installer
# Usage: curl -fsSL https://raw.githubusercontent.com/HKUDS/Codeless/main/scripts/install.sh | bash
...
# ---------------------------------------------------------------------------
step "Installing Codeless"

REPO_URL="https://github.com/HKUDS/Codeless.git"
INSTALL_DIR="$HOME/.codeless-src"
VENV_DIR="$HOME/.codeless-venv"
...
if [ "$FROM_SOURCE" = true ]; then
    ...
    if command -v git &>/dev/null; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            info "Cloning Codeless into ${INSTALL_DIR}..."
    ...
    info "Mode: pip install codeless-ai"
    $PIP_CMD install codeless-ai --quiet --upgrade
fi

success "Codeless package installed"
...
# ---------------------------------------------------------------------------
# Step 7: Create Codeless config directory
# ---------------------------------------------------------------------------
step "Setting up Codeless config directory"

mkdir -p "$HOME/.codeless"
mkdir -p "$HOME/.codeless/skills"
mkdir -p "$HOME/.codeless/plugins"

success "Config directory ready: ~/.codeless/"
...
# ---------------------------------------------------------------------------
echo ""
echo -e "${BOLD}${GREEN}Codeless is installed!${RESET}"
echo ""
echo "  Next steps:"
