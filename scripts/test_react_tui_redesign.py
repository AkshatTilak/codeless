...
def test_welcome_banner() -> tuple[bool, str]:
    """Test that the React TUI shows 'CODELESS' on startup."""
    try:
        import pexpect
    except ImportError:
    ...
    env["CODELESS_FRONTEND_SCRIPT"] = json.dumps(["/exit"])

    backend_cmd = [sys.executable, "-m", "codeless", "--backend-only"]
    ...
    env["CODELESS_FRONTEND_CONFIG"] = frontend_config
    ...
    try:
        ...
        child.expect("CODELESS", timeout=15)
        child.expect(pexpect.EOF, timeout=15)
        return True, "Welcome banner displayed with 'CODELESS'"
    except pexpect.TIMEOUT:
        output = child.before or ""
        return False, f"Timeout waiting for welcome banner. Output: {output[:300]}"
    except pexpect.EOF:
        output = child.before or ""
        if "CODELESS" in output:
...
def test_conversation_flow() -> tuple[bool, str]:
    ...
    env = _env()
    env["CODELESS_FRONTEND_RAW_RETURN"] = "1"
    env["CODELESS_FRONTEND_SCRIPT"] = json.dumps(["Say exactly: hello world", "/exit"])
...
def test_status_bar() -> tuple[bool, str]:
    ...
    env = _env()
    env["CODELESS_FRONTEND_RAW_RETURN"] = "1"
    env["CODELESS_FRONTEND_SCRIPT"] = json.dumps(["Say hi", "/exit"])

    model_name = env.get("ANTHROPIC_MODEL", "kimi-k2.5")
    backend_cmd = [sys.executable, "-m", "codeless", "--backend-only", "--model", model_name]
    frontend_config = json.dumps(
        {
            "backend_command": backend_cmd,
            "initial_prompt": None,
