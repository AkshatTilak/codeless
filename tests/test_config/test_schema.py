...
class TestTelegramConfig:
    def test_reply_to_message_field_is_declared_with_true_default(self):
        ...
        existed only as an interactive ``codeless init`` prompt, not on the
    ...
    def test_reply_to_message_accessible_when_legacy_config_omits_field(self):
        """``codeless init --no-interactive`` and pre-0.1.9 hand-written
    ...
    def test_bot_name_defaults_to_codeless(self):
        config = TelegramConfig()

        assert config.bot_name == "codeless"
