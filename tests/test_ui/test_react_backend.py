...
from codeless.ui.protocol import BackendEvent, FrontendImageAttachment, FrontendRequest
...
async def test_run_backend_host_accepts_permission_mode(monkeypatch):
    ...
    monkeypatch.setattr("codeless.ui.backend_host.ReactBackendHost.run", _fake_run)
...
async def test_read_requests_resolves_permission_response_without_queueing(monkeypatch):
    ...
    monkeypatch.setattr("codeless.ui.backend_host.sys.stdin", _FakeStdin())
...
async def test_backend_host_processes_command(tmp_path, monkeypatch):
    ...
    assert any(
        ...
        and event.item
        and event.item.role == "system"
        and "Codeless" in event.item.text
        for event in events
    )
...
async def test_backend_host_command_does_not_reset_cli_overrides(tmp_path, monkeypatch):
    ...
    preserve the effective session settings, not reload ~/.codeless/settings.json
    verbatim.
    """
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("CODELESS_CONFIG_DIR", str(tmp_path / "config"))
...
async def test_backend_host_uses_effective_model_from_env_override(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CODELESS_CONFIG_DIR", str(tmp_path / "config"))
    monkeypatch.setenv("CODELESS_DATA_DIR", str(tmp_path / "data"))
    monkeypatch.setenv("CODELESS_MODEL", "minimax-m1")
...
async def test_backend_host_emits_utf8_protocol_bytes(monkeypatch):
    ...
    monkeypatch.setattr("codeless.ui.backend_host.sys.stdout", fake_stdout)
