...
def test_build_backend_command_includes_flags():
    ...
    assert command[:3] == [command[0], "-m", "codeless"]
...
async def test_run_repl_uses_react_launcher_by_default(monkeypatch):
    ...
    monkeypatch.setattr("codeless.ui.app.launch_react_tui", _launch)
...
async def test_run_print_mode_passes_cwd_to_build_runtime(monkeypatch):
    ...
    monkeypatch.setattr("codeless.ui.app.build_runtime", _build_runtime)
    monkeypatch.setattr("codeless.ui.app.start_runtime", _start_runtime)
    monkeypatch.setattr("codeless.ui.app.handle_line", _handle_line)
    monkeypatch.setattr("codeless.ui.app.close_runtime", _close_runtime)
...
async def test_run_task_worker_reads_one_shot_json_line(monkeypatch):
    ...
    monkeypatch.setattr("codeless.ui.app.handle_line", _handle_line)
    monkeypatch.setattr("codeless.ui.app.close_runtime", _close_runtime)
    monkeypatch.setattr("codeless.ui.app.sys.stdin", _FakeStdin())
...
async def test_run_print_mode_waits_for_coordinator_async_agents(monkeypatch):
    ...
    monkeypatch.setattr("codeless.ui.app.handle_line", _handle_line)
    monkeypatch.setattr("codeless.ui.app.close_runtime", _close_runtime)
    monkeypatch.setattr("codeless.ui.coordinator_drain.get_task_manager", lambda: fake_manager)
    monkeypatch.setattr("codeless.ui.app.is_coordinator_mode", lambda: True)
    monkeypatch.setattr(
        "codeless.ui.coordinator_drain.build_runtime_system_prompt",
    ...
    monkeypatch.setattr("codeless.ui.coordinator_drain.asyncio.sleep", _sleep)
