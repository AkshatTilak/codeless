"""Tests for codeless.prompts.system_prompt."""

from __future__ import annotations

from codeless.prompts.environment import EnvironmentInfo
...
def _make_env(**overrides) -> EnvironmentInfo:
    defaults = dict(
        ...
        python_executable="/home/user/.codeless-venv/bin/python",
        virtual_env="/home/user/.codeless-venv",
        is_git_repo=True,
        git_branch="main",
        hostname="testhost",
    )
...
def test_build_system_prompt_contains_environment():
    ...
    assert "/home/user/.codeless-venv/bin/python" in prompt
    assert "Virtual environment: /home/user/.codeless-venv" in prompt
...
def test_build_system_prompt_custom_prompt():
    ...
    assert "Codeless" not in prompt


def test_build_system_prompt_default_includes_base():
    env = _make_env()
    prompt = build_system_prompt(env=env)
    assert "Codeless" in prompt
