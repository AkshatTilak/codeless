...
def test_anthropic_client_adds_oauth_beta_header(monkeypatch):
    ...
    monkeypatch.setattr("codeless.api.client.AsyncAnthropic", _FakeAsyncAnthropic)
...
def test_anthropic_client_adds_claude_oauth_identity_headers(monkeypatch):
    ...
    monkeypatch.setattr("codeless.api.client.AsyncAnthropic", _FakeAsyncAnthropic)
    monkeypatch.setattr(
        "codeless.auth.external.get_claude_code_version",
        lambda: "2.1.92",
    )
    monkeypatch.setattr(
        "codeless.auth.external.get_claude_code_session_id",
        lambda: "session-123",
    )
    monkeypatch.setattr(
        "codeless.api.client.get_claude_code_session_id",
        lambda: "session-123",
    )
    monkeypatch.setattr(
        "codeless.api.client.claude_attribution_header",
...
def test_anthropic_client_refreshes_claude_token_on_request(monkeypatch):
    ...
    assert client._client.beta.messages.last_params["metadata"] == {
        "user_id": '{"device_id":"codeless","session_id":"session-123","account_uuid":""}'
    }
