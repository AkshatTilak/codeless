...
from codeless.hooks.loader import load_hook_registry
from codeless.plugins import load_plugins
from codeless.plugins.loader import get_user_plugins_dir
...
def test_load_plugins_from_project_dir(tmp_path: Path, monkeypatch):
    ...
    plugins_root = project / ".codeless" / "plugins"
...
def test_user_plugins_still_load_when_project_plugins_are_disabled(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("CODELESS_CONFIG_DIR", str(tmp_path / "config"))
    project = tmp_path / "repo"
    project_plugins_root = project / ".codeless" / "plugins"
