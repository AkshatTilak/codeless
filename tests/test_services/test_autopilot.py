...
from codeless.autopilot import RepoAutopilotStore, RepoVerificationStep
from codeless.autopilot.service import _DEFAULT_VERIFICATION_POLICY
...
def test_autopilot_pick_next_prefers_highest_score(tmp_path: Path) -> None:
    ...
    assert next_card.source_kind == "remote_request"
...
def test_autopilot_tick_scans_then_runs_next(tmp_path: Path) -> None:
    ...
    async def fake_run_next(self, *, model=None, max_turns=None, permission_mode=None):
        from codeless.autopilot import RepoRunResult
...
def test_autopilot_install_default_cron_creates_jobs(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "codeless.services.cron.upsert_cron_job",
...
def test_wait_for_pr_ci_allows_repos_with_no_remote_checks_after_grace(
    ...
    monkeypatch.setattr("codeless.autopilot.service.time.time", lambda: next(times))
    monkeypatch.setattr(
        "codeless.autopilot.service.asyncio.sleep",
...
def test_wait_for_pr_ci_waits_for_check_settle_window(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("codeless.autopilot.service.time.time", lambda: current_time["value"])
    snapshots = [
        {
    ...
    monkeypatch.setattr("codeless.autopilot.service.asyncio.sleep", fake_sleep)
    monkeypatch.setattr(
        store,
        "_pr_status_snapshot",
