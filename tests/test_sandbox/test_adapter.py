...
from codeless.config.settings import SandboxSettings, Settings
from codeless.sandbox.adapter import (
...
def test_sandbox_availability_reports_native_windows_unsupported(monkeypatch):
    ...
    monkeypatch.setattr("codeless.sandbox.adapter.get_platform", lambda: "windows")
...
def test_wrap_command_for_sandbox_writes_settings_file(monkeypatch):
    ...
    monkeypatch.setattr("codeless.sandbox.adapter.get_platform", lambda: "linux")
    monkeypatch.setattr("codeless.sandbox.adapter.shutil.which", fake_which)
...
def test_wrap_command_for_sandbox_raises_when_required(monkeypatch):
    ...
    monkeypatch.setattr("codeless.sandbox.adapter.get_platform", lambda: "linux")
    monkeypatch.setattr("codeless.sandbox.adapter.shutil.which", lambda name: None)
...
def test_create_shell_subprocess_preserves_exit_code_with_sandbox(monkeypatch):
    import codeless.config.paths as config_paths

    async def _run() -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            cfg = Path(tmpdir) / "settings.json"
            from codeless.config.settings import save_settings
            ...
            try:
                process = await create_shell_subprocess(
                    "exit 7",
                    cwd=Path("/home/tangjiabin/Codeless-new"),
