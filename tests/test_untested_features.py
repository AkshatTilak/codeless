"""Comprehensive integration tests for all previously untested Codeless features.
    ...
    for ev in events:
        if isinstance(ev, AssistantTextDelta):
            ...
            r["tool_results"].append(
                {"tool": ev.tool_name, "ok": not ev.is_error, "out": ev.output[:300]}
            )
...
async def test_hooks_command_block():
    ...
    from codeless.hooks.executor import HookExecutionContext, HookExecutor
    from codeless.hooks.loader import HookRegistry
    from codeless.hooks.schemas import CommandHookDefinition
...
async def test_hooks_in_agent_loop():
    ...
    from codeless.api.client import AnthropicApiClient
    from codeless.config.settings import PermissionSettings
    from codeless.engine.messages import ConversationMessage
    from codeless.engine.query import QueryContext, run_query
    from codeless.engine.stream_events import (
        AssistantTextDelta,
        ToolExecutionCompleted,
        ToolExecutionStarted,
    ...
    hook_executor = HookExecutor(
        hook_reg, HookExecutionContext(cwd=WORKSPACE, api_client=api, default_model=MODEL)
    )
    reg = ToolRegistry()
...
async def test_skills_load():
    ...
    from codeless.skills.loader import load_user_skills
...
""")
        ...
        import codeless.skills.loader as sl
...
async def test_plugins_load():
    ...
    from codeless.plugins.loader import load_plugin
...
async def test_memory_lifecycle():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        import codeless.memory.paths as mp

        orig = mp.get_project_memory_dir
        mem_dir = Path(tmpdir) / ".codeless" / "memory"
        mem_dir.mkdir(parents=True, exist_ok=True)
        mp.get_project_memory_dir = lambda cwd: mem_dir

        # Also patch entrypoint
        import codeless.memory.manager as mm
        ...
        p2 = add_memory_entry(
            tmpdir, "project-goal", "Building an AI agent framework called Codeless"
        )
...
async def test_session_storage():
    ...
    from codeless.services.session_storage import (
...
async def test_config_settings():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(
            f"  Loaded from file: model={loaded.model}, perm={loaded.permission.mode}, memory={loaded.memory.enabled}"
        )
...
async def test_commands_registry():
    ...
    print(
        f"  Lookup '/test hello world': {found[0].name if found else 'NOT FOUND'}, args='{found[1] if found else ''}'"
    )
    ...
    # Default registry
    from codeless.commands.registry import create_default_command_registry
    ...
    return (
        found is not None
        and found[0].name == "test"
        and found[1] == "hello world"
...
async def test_worktree_real_git():
    ...
    from codeless.swarm.worktree import WorktreeManager
...
async def test_config_paths():
    ...
    # All should be under ~/.codeless
    all_under_home = all(".codeless" in str(p) for p in paths.values())
...
async def test_combined_hooks_skills_agent():
    ...
    from codeless.engine.messages import ConversationMessage
    from codeless.engine.query import QueryContext, run_query
    from codeless.engine.stream_events import AssistantTextDelta, ToolExecutionStarted
    from codeless.hooks.events import HookEvent
    from codeless.hooks.executor import HookExecutionContext, HookExecutor
    from codeless.hooks.loader import HookRegistry
    from codeless.hooks.schemas import CommandHookDefinition
...
async def test_full_swarm_autoagent():
    ...
    import codeless.swarm.mailbox as mb
    import codeless.swarm.team_lifecycle as tl
    from codeless.api.client import AnthropicApiClient
    from codeless.config.settings import PermissionSettings
    from codeless.engine.query import QueryContext
    from codeless.permissions.checker import PermissionChecker
    from codeless.permissions.modes import PermissionMode
    from codeless.swarm.in_process import TeammateAbortController, start_in_process_teammate
    from codeless.swarm.team_lifecycle import TeamLifecycleManager, TeamMember
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            mgr.add_member(
                "autoagent-research",
                TeamMember(
                    agent_id="leader@autoagent-research",
                    name="leader",
            ...
            async def run_teammate(name, prompt):
                ...
                mgr.add_member(
                    "autoagent-research",
                    TeamMember(
                        agent_id=f"{name}@autoagent-research",
                        name=name,
                        backend_type="in_process",
                        joined_at=time.time(),
                        is_active=True,
            ...
            results = await asyncio.gather(
                asyncio.wait_for(
                    run_teammate(
                        "counter",
                        "Count .py files in autoagent/ using bash: find autoagent -name '*.py' | wc -l",
                    ),
                    timeout=30,
                ),
                asyncio.wait_for(
                    run_teammate(
                        "finder",
                        "Find the main entry point of AutoAgent using grep: grep -rn 'def main' autoagent/",
                    ),
                    timeout=30,
                ),
            ...
            print(
                f"  Worker results: {['OK' if not isinstance(r, Exception) else str(r) for r in results]}"
            )
