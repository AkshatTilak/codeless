"""Path resolution for Codeless configuration and data directories.

Follows XDG-like conventions with ~/.codeless/ as the default base directory.
"""
...
def get_config_dir() -> Path:
    ...
    Resolution order:
    1. CODELESS_CONFIG_DIR environment variable
    2. ~/.codeless/
    """
    env_dir = os.environ.get("CODELESS_CONFIG_DIR")
    if env_dir:
...
def get_config_file_path() -> Path:
    """Return the path to the main settings file (~/.codeless/settings.json)."""
...
def get_data_dir() -> Path:
    ...
    Resolution order:
    1. CODELESS_DATA_DIR environment variable
    2. ~/.codeless/data/
    """
    env_dir = os.environ.get("CODELESS_DATA_DIR")
    if env_dir:
...
def get_logs_dir() -> Path:
    ...
    Resolution order:
    1. CODELESS_LOGS_DIR environment variable
    2. ~/.codeless/logs/
    """
    env_dir = os.environ.get("CODELESS_LOGS_DIR")
    if env_dir:
...
def get_project_config_dir(cwd: str | Path) -> Path:
    """Return the per-project .codeless directory."""
    project_dir = Path(cwd).resolve() / ".codeless"
