"""Settings model and loading logic for Codeless.

Settings are resolved with the following precedence (highest first):
1. CLI arguments
2. Environment variables (ANTHROPIC_API_KEY, CODELESS_MODEL, etc.)
3. Config file (~/.codeless/settings.json)
4. Defaults
"""
...
from codeless.mcp.types import McpServerConfig
...
class DockerSandboxSettings(BaseModel):
    ...
    image: str = "codeless-sandbox:latest"
...
def auth_source_env_var_candidates(auth_source: str) -> tuple[str, ...]:
    ...
    mapping = {
        "anthropic_api_key": ("CODELESS_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY"),
        "openai_api_key": ("CODELESS_OPENAI_API_KEY", "OPENAI_API_KEY"),
        "dashscope_api_key": ("CODELESS_DASHSCOPE_API_KEY", "DASHSCOPE_API_KEY"),
        "moonshot_api_key": ("CODELESS_MOONSHOT_API_KEY", "MOONSHOT_API_KEY"),
        "gemini_api_key": ("CODELESS_GEMINI_API_KEY", "GEMINI_API_KEY"),
        "minimax_api_key": ("CODELESS_MINIMAX_API_KEY", "MINIMAX_API_KEY"),
        "nvidia_api_key": ("CODELESS_NVIDIA_API_KEY", "NVIDIA_API_KEY"),
        "modelscope_api_key": ("CODELESS_MODELSCOPE_API_KEY", "MODELSCOPE_API_KEY"),
    }
...
class ImageGenerationConfig(BaseModel):
    ...
    def from_env(cls) -> "ImageGenerationConfig":
        ...
        return cls(
            ...
            base_url=os.environ.get("CODELESS_IMAGE_GENERATION_BASE_URL", "").strip(),
            codex_model=os.environ.get("CODELESS_IMAGE_GENERATION_CODEX_MODEL", "gpt-5.4").strip()
            or "gpt-5.4",
            codex_base_url=os.environ.get("CODELESS_IMAGE_GENERATION_CODEX_BASE_URL", "").strip(),
        )
...
class VisionModelConfig(BaseModel):
    ...
    def from_env(cls) -> "VisionModelConfig":
        ...
        return cls(
            model=os.environ.get("CODELESS_VISION_MODEL", "").strip(),
            api_key=os.environ.get("CODELESS_VISION_API_KEY", "").strip(),
            base_url=os.environ.get("CODELESS_VISION_BASE_URL", "").strip(),
        )
...
class Settings(BaseModel):
    """Main settings model for Codeless."""
    ...
    project_skill_dirs: list[str] = Field(
        default_factory=lambda: [".codeless/skills", ".agents/skills", ".claude/skills"]
    )
    ...
    def sync_active_profile_from_flat_fields(self) -> Settings:
        ...
        """
        profile_name, profile = self.resolve_profile()
        profile_from_env = bool(os.environ.get("CODELESS_PROFILE"))
    ...
    def resolve_api_key(self) -> str:
        ...
        raise ValueError(
            "No API key found. Set an CODELESS_* provider API key "
            "(preferred) or the matching native provider environment variable, "
            "or configure api_key in ~/.codeless/settings.json"
        )
    ...
    def resolve_auth(self) -> ResolvedAuth:
        ...
        if auth_source in {"codex_subscription", "claude_subscription"}:
            ...
            from codeless.auth.external import (
...
def _apply_env_overrides(settings: Settings) -> Settings:
    ...
    explicitly configure the corresponding field.  ``CODELESS_*`` env vars
    ...
    if codeless_model:
        updates["model"] = strip_ansi_escape_sequences(codeless_model)
    ...
    # --- base_url ---
    codeless_base = os.environ.get("CODELESS_BASE_URL")
    if codeless_base:
        updates["base_url"] = codeless_base
    ...
    max_tokens = os.environ.get("CODELESS_MAX_TOKENS")
    if max_tokens:
        updates["max_tokens"] = int(max_tokens)

    timeout = os.environ.get("CODELESS_TIMEOUT")
    if timeout:
        updates["timeout"] = float(timeout)

    max_turns = os.environ.get("CODELESS_MAX_TURNS")
    if max_turns:
        updates["max_turns"] = int(max_turns)

    context_window_tokens = os.environ.get("CODELESS_CONTEXT_WINDOW_TOKENS")
    if context_window_tokens:
        updates["context_window_tokens"] = int(context_window_tokens)

    auto_compact_threshold_tokens = os.environ.get("CODELESS_AUTO_COMPACT_THRESHOLD_TOKENS")
    if auto_compact_threshold_tokens:
        updates["auto_compact_threshold_tokens"] = int(auto_compact_threshold_tokens)

    provider = os.environ.get("CODELESS_PROVIDER")
    api_format = os.environ.get("CODELESS_API_FORMAT")
    ...
    sandbox_enabled = os.environ.get("CODELESS_SANDBOX_ENABLED")
    sandbox_fail = os.environ.get("CODELESS_SANDBOX_FAIL_IF_UNAVAILABLE")
    sandbox_backend = os.environ.get("CODELESS_SANDBOX_BACKEND")
    sandbox_docker_image = os.environ.get("CODELESS_SANDBOX_DOCKER_IMAGE")
    ...
    web_proxy = os.environ.get("CODELESS_WEB_PROXY")
    if web_proxy:
        web_updates["proxy"] = web_proxy
    web_resolution_mode = os.environ.get("CODELESS_WEB_RESOLUTION_MODE")
...
def load_settings(config_path: Path | None = None) -> Settings:
    ...
    if config_path is None:
        from codeless.config.paths import get_config_file_path
    ...
    if config_path.exists():
        ...
        env_profile = os.environ.get("CODELESS_PROFILE")
        if env_profile:
