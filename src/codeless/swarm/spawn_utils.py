...
_TEAMMATE_ENV_VARS = [
    ...
    "REQUESTS_CA_BUNDLE",
    "CURL_CA_BUNDLE",
    # --- Codeless-native provider settings --------------------------------
    ...
    "CODELESS_BASE_URL",
    "CODELESS_MODEL",
    "CODELESS_ANTHROPIC_API_KEY",
    "CODELESS_OPENAI_API_KEY",
    "CODELESS_DASHSCOPE_API_KEY",
    "CODELESS_MOONSHOT_API_KEY",
    "CODELESS_GEMINI_API_KEY",
    "CODELESS_MINIMAX_API_KEY",
    "CODELESS_NVIDIA_API_KEY",
    "CODELESS_MODELSCOPE_API_KEY",
    "OPENAI_API_KEY",
]
...
def get_teammate_command() -> str:
    ...
    Resolution order:
    1. ``CODELESS_TEAMMATE_COMMAND`` environment variable — allows the
       operator to point at a specific binary or wrapper script.
    2. The current Python interpreter running the ``codeless`` module.
       This keeps spawned teammates on the same venv/source tree as the
       leader process.
    3. The ``codeless`` entry-point on PATH (installed package fallback).
    """
    ...
    entry_point = shutil.which("codeless")
    if entry_point:
        return entry_point
    return "python"
...
def build_inherited_cli_flags(
    *,
    ...
    # --- Model override ----------------------------------------------------
    # "inherit" means use the parent's model via the CODELESS_MODEL env var.
...
def build_inherited_env_vars() -> dict[str, str]:
    ...
    Always includes ``CODELESS_AGENT_TEAMS=1`` plus any provider/proxy
