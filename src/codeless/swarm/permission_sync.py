"""Permission sync protocol for leader-worker coordination in Codeless swarms.
...
Paths:
    ~/.codeless/teams/<teamName>/permissions/pending/<id>.json
    ~/.codeless/teams/<teamName>/permissions/resolved/<id>.json
"""
...
def get_permission_dir(team_name: str) -> Path:
    """Return ~/.codeless/teams/{teamName}/permissions/"""
...
async def get_leader_name(team_name: str | None = None) -> str | None:
    ...
    """
    from codeless.swarm.team_lifecycle import read_team_file_async
...
async def handle_permission_request(
    ...
    Args:
        ...
        checker: An already-configured :class:`~codeless.permissions.checker.PermissionChecker`.
