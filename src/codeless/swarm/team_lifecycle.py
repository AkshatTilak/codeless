"""Persistent team lifecycle management for Codeless swarms.

Teams are stored as JSON files on disk:
    ~/.codeless/teams/<name>/team.json
...
from codeless.swarm.mailbox import get_team_dir
from codeless.swarm.types import BackendType
...
async def _kill_orphaned_teammate_panes(team_name: str) -> None:
    ...
    from codeless.swarm.spawn_utils import is_inside_tmux
    from codeless.swarm.types import is_pane_backend
...
class TeamLifecycleManager:
    ...
    Persists team metadata to ``~/.codeless/teams/<name>/team.json``.
    ...
    def list_teams(self) -> list[TeamFile]:
        """Return all teams found in ``~/.codeless/teams/``, sorted by name."""
        base = Path.home() / ".codeless" / "teams"
        if not base.exists():
            return []
