...
from codeless.api.client import AnthropicApiClient, SupportsStreamingMessages
...
def _resolve_image_generation_config(settings) -> dict[str, str]:
    ...
    from codeless.config.settings import ImageGenerationConfig, ProviderProfile
...
class RuntimeBundle:
    ...
    def current_settings(self):
        ...
        We persist most settings to disk (``~/.codeless/settings.json``), but
...
def _resolve_api_client_from_settings(settings) -> SupportsStreamingMessages:
    ...
    if settings.api_format == "copilot":
        from codeless.api.copilot_client import COPILOT_DEFAULT_MODEL
...
async def build_runtime(
    *,
    ...
    """Build the shared runtime for an Codeless session."""
    ...
    if settings.sandbox.enabled and settings.sandbox.backend == "docker":
        from codeless.sandbox.session import start_docker_sandbox
...
async def close_runtime(bundle: RuntimeBundle) -> None:
    ...
    from codeless.sandbox.session import stop_docker_sandbox
