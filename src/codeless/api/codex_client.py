...
def _build_codex_headers(token: str, *, session_id: str | None = None) -> dict[str, str]:
    ...
    headers = {
        ...
        "originator": "codeless",
        "User-Agent": f"codeless ({platform.system().lower()} {platform.machine() or 'unknown'})",
...
def _translate_status_error(status_code: int, message: str) -> CodelessApiError:
...
class CodexApiClient:
    ...
    async def _stream_once(self, request: ApiMessageRequest) -> AsyncIterator[ApiStreamEvent]:
        body: dict[str, Any] = {
            ...
            "store": False,
            "stream": True,
            "instructions": request.system_prompt or "You are Codeless.",
    ...
    @staticmethod
    def _translate_error(exc: Exception) -> CodelessApiError:
        if isinstance(exc, CodelessApiError):
            return exc
        if isinstance(exc, httpx.HTTPStatusError):
