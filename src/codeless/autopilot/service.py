...
from codeless.engine.stream_events import AssistantTextDelta, AssistantTurnComplete, ErrorEvent
...
class RepoAutopilotStore:
    ...
    def pick_next_card(self) -> RepoTaskCard | None:
        ...
        if not queued:
            return None
        return sorted(queued, key=lambda card: (-card.score, -card.updated_at, card.title.lower()))[
            0
        ]
    ...
    def rebuild_active_context(self) -> str:
        ...
        for card in sorted(running + accepted, key=lambda item: (-item.score, -item.updated_at))[
            :6
        ]:
        ...
        for label, path in discovered[:limit]:
            ...
            card, _ = self.enqueue_card(
                ...
                body=(
                    ...
                    "Review whether this should be aligned, adapted, or ignored for Codeless."
                ),
    ...
    async def run_card(
        ...
        *,
        ...
        effective_max_turns = (
            max_turns if max_turns is not None else int(execution.get("max_turns", 12))
        )
    ...
    async def tick(
        ...
        *,
        ...
        if any(
            card.status in {"preparing", "running", "verifying", "waiting_ci", "repairing"}
            for card in self.list_cards()
        ):
            self.append_journal(
                kind="tick_skip", summary="Skipped run-next because another card is active"
            )
            return None
    ...
    def install_default_cron(self) -> list[str]:
        from codeless.services.cron import upsert_cron_job
    ...
    def _sync_worktree_to_base(
        self, cwd: Path, *, base_branch: str, head_branch: str, reset: bool
    ) -> None:
    ...
    def _comment_on_pr(self, pr_number: int, comment: str) -> None:
        try:
            self._run_gh(
                ["pr", "comment", str(pr_number), "--body", comment], cwd=self._cwd, check=True
            )
        except Exception as exc:
            self.append_journal(
                kind="github_warning",
    ...
    def _comment_started(self, card: RepoTaskCard, attempt_count: int) -> str:
        return _bilingual_lines(
            f"Codeless autopilot 已开始处理 `{card.id}`，当前第 {attempt_count} 轮执行。",
            f"Codeless autopilot started processing `{card.id}`. Attempt {attempt_count} is now running.",
        )
    ...
    def _render_dashboard_html(self, snapshot: dict[str, Any]) -> str:
        ...
        """
        repo_name = escape(_safe_text(snapshot.get("repo_name")) or "Codeless")
