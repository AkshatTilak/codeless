...
name: skill-creator
description: >
  Create, improve, and verify Codeless skills. Use this whenever the user
  asks to create a new skill, convert a workflow into a skill, update an
  existing SKILL.md, add skills for Codeless, design skill trigger behavior,
  or test whether a skill loads and works correctly.
---

# skill-creator

Create or improve Codeless skills in the directory-based `SKILL.md` format.
...
- make a skill available to `codeless`, `clh`, or a plugin
- debug why the `skill` tool cannot find or load a skill
- test trigger wording, skill metadata, or skill behavior

## Codeless skill locations

Choose the target deliberately:

- Built-in Codeless skills live in `src/codeless/skills/bundled/content/*.md`.
- User skills for `codeless` live in `~/.codeless/skills/<skill-dir>/SKILL.md`.
- Project/ABB skills live in `.codeless/abb_workspace/skills/<skill-dir>/SKILL.md`.
- Plugin skills live in `<plugin-root>/skills/<skill-dir>/SKILL.md`.

Codeless user and plugin skills use a directory layout. Do not create flat
...
2. Inspect existing examples.
   ...
   Codeless, confirm whether the target is bundled, user, project, or
   plugin-provided.
...
6. Verify loading.
   ...
   For custom/user skills, include the extra skill directory:

   ```bash
   PYTHONPATH=src python - <<'PY'
   from codeless.skills import load_skill_registry
   from codeless.config.paths import get_skills_dir
