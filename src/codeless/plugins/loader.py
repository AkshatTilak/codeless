...
from codeless.plugins.types import LoadedPlugin, PluginCommandDefinition
from codeless.skills.loader import _parse_skill_metadata
...
def get_project_plugins_dir(cwd: str | Path) -> Path:
    ...
    path = Path(cwd).resolve() / ".codeless" / "plugins"
    path.mkdir(parents=True, exist_ok=True)
    return path
...
def _load_plugin_mcp(path: Path) -> dict[str, object]:
    ...
    if not path.exists():
        return {}
    from codeless.mcp.types import McpJsonConfig
...
def _load_plugin_tools(path: Path, manifest: PluginManifest) -> list:
    ...
    from codeless.tools.base import BaseTool
